...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Sidebar init (reuse from app.js if available)
  initSidebarEmp();
  initTopbarEmp();
  ...
  if (uBtn && uDrop) {
    uBtn.addEventListener("click", function (e) { e.stopPropagation(); uDrop.classList.toggle("open"); });
    uDrop.addEventListener("click", e => e.stopPropagation());
  }
  document.addEventListener("click", function () { if (uDrop) uDrop.classList.remove("open"); });
...
function renderOverview() {
  ...
  const resigned  = 0; // no resigned status in current data
  const depts     = [...new Set(emps.map(e => e.dept).filter(Boolean))].length;

  // Confirmation due (probation employees within 90 days of joining)
  const confirmDue = emps.filter(e => {
    ...
    confirmDate.setDate(confirmDate.getDate() + 90);
    const diff = Math.round((confirmDate - today) / (1000 * 60 * 60 * 24));
...
function renderHeadcountChart(emps) {
  ...
  // Cumulative count per month (FY 2025-26: Apr2025 - Mar2026)
...
function renderGenderChart(emps) {
  ...
  const other  = emps.filter(e => e.gender === "Other").length;
  const total  = emps.length;
  ...
  if (legendEl) {
    legendEl.innerHTML = [
      ...
      <div class="ov-gender-legend-item">
        ...
        <span style="margin-left:auto;color:var(--muted);">${Math.round(g.val/total*100)}% (${g.val})</span>
      </div>
    `).join("");
  }
...
function renderConfirmationDue(emps, today) {
  ...
  const list = emps
    ...
    .map(e => {
      const doj  = new Date(e.doj);
      const conf = new Date(doj); conf.setDate(conf.getDate() + 90);
  ...
  el.innerHTML = list.map(e => {
    const cls = e.diff < 0 ? "urgent" : e.diff <= 7 ? "urgent" : e.diff <= 15 ? "soon" : "ok";
...
function populateFilterDropdowns() {
  ...
  if (dEl) {
    dEl.innerHTML = `<option value="">Department: All</option>` +
      depts.map(d => `<option value="${d}">${d}</option>`).join("");
  }
  if (dgEl) {
    dgEl.innerHTML = `<option value="">Designation: All</option>` +
...
function filterDirectory() {
  ...
  filteredEmployees = emps.filter(e => {
    ...
    const matchQ  = !q || name.includes(q) || empCode.includes(q) || email.includes(q);
    const matchD  = !dept   || e.dept   === dept;
    const matchDg = !desig  || e.desig  === desig;
    const matchS  = !status || e.status === status;
    return matchQ && matchD && matchDg && matchS;
  });
  dirPage = 1;
  renderDirTable();
}
...
function renderDirTable() {
  ...
  if (!slice.length) {
    ...
    tbody.innerHTML = slice.map((e, i) => {
      const badgeCls = e.status === "Active" ? "status-active" :
      ...
      return `<tr>
        ...
        <td><span class="status-badge ${badgeCls}">${e.status}</span></td>
        <td>
...
function renderProfile() {
  ...
  if (!currentProfileId) {
    ...
    document.querySelector(".prof-back-row").style.display = "none";
    return;
  }
  ...
  document.querySelector(".prof-back-row").style.display = "";
  ...
  if (meta) {
    ...
    const dojD = emp.doj ? new Date(emp.doj) : null;
    meta.innerHTML = [
      emp.empCode || emp.id,
      emp.gender,
      ...
      emp.desig,
      emp.dept,
      dojD ? `Joined on ${formatDisplayDate(emp.doj)}` : null
    ].filter(Boolean).map((t,i,arr) =>
      `<span class="prof-meta-item">${t}</span>${i < arr.length-1 ? '<span class="prof-meta-dot"></span>' : ""}`
    ).join("");
  }
...
function fields(pairs) {
  return pairs.map(([label, val]) => `
    <div class="prof-field">
      ...
      <span class="prof-field-val">${val || "—"}</span>
    </div>
  `).join("");
}
...
function openEditDrawerById(id) {
  ...
  drawerStep = 1;
  currentProfileId = id;
  document.getElementById("drawerTitle").textContent = `Edit Employee — ${emp.firstName} ${emp.lastName}`;
  document.getElementById("drawerCancelBtn").textContent = "Back";
  document.getElementById("drawerNextBtn").textContent = "Next";
  fillDrawerForm(emp);
  updateDrawerStepUI();
  openDrawer();
...
function fillDrawerForm(emp) {
  setVal("df-empCode",     emp.empCode || emp.id);
  setVal("df-firstName",   emp.firstName);
  setVal("df-lastName",    emp.lastName);
  setVal("df-gender",      emp.gender);
  setVal("df-dob",         emp.dob);
  setVal("df-marital",     emp.marital);
  setVal("df-blood",       emp.blood);
  setVal("df-email",       emp.email);
  setVal("df-mobile",      emp.mobile);
  setVal("df-emergency",   emp.emergency);
  setVal("df-currentAddr", emp.currentAddr);
  setVal("df-permAddr",    emp.permAddr);
  setVal("df-dept",        emp.dept);
  setVal("df-desig",       emp.desig);
  setVal("df-doj",         emp.doj);
  setVal("df-empType",     emp.empType);
  setVal("df-status",      emp.status);
  setVal("df-manager",     emp.manager);
  setVal("df-location",    emp.location);
  setVal("df-ctc",         emp.ctc);
  setVal("df-pan",         emp.pan);
  setVal("df-aadhaar",     emp.aadhaar);
  setVal("df-uan",         emp.uan);
  setVal("df-esi",         emp.esi);
  setVal("df-bank",        emp.bank);
  setVal("df-ifsc",        emp.ifsc);
}

function clearDrawerForm() {
  const ids = ["df-empCode","df-firstName","df-lastName","df-gender","df-dob",
    "df-marital","df-blood","df-email","df-mobile","df-emergency",
    "df-currentAddr","df-permAddr","df-dept","df-desig","df-doj",
    "df-empType","df-status","df-manager","df-location","df-ctc",
...
function validateDrawerStep(step) {
  const required = {
    1: ["df-empCode","df-firstName","df-lastName","df-gender","df-dob"],
    2: ["df-email","df-mobile"],
    3: ["df-dept","df-desig","df-doj"]
  };
...
function updateDrawerStepUI() {
  ...
  // Panels
  document.querySelectorAll(".drawer-step-panel").forEach(function (p) {
...
function saveDrawerEmployee() {
  const emps = getEmployees();
  const data = {
    empCode:     getVal("df-empCode"),
    firstName:   getVal("df-firstName"),
    lastName:    getVal("df-lastName"),
    gender:      getVal("df-gender"),
    dob:         getVal("df-dob"),
    marital:     getVal("df-marital"),
    blood:       getVal("df-blood"),
    email:       getVal("df-email"),
    mobile:      getVal("df-mobile"),
    emergency:   getVal("df-emergency"),
    currentAddr: getVal("df-currentAddr"),
    permAddr:    getVal("df-permAddr"),
    dept:        getVal("df-dept"),
    desig:       getVal("df-desig"),
    doj:         getVal("df-doj"),
    empType:     getVal("df-empType"),
    status:      getVal("df-status") || "Active",
    manager:     getVal("df-manager"),
    location:    getVal("df-location"),
    ctc:         getVal("df-ctc"),
    pan:         getVal("df-pan"),
    aadhaar:     getVal("df-aadhaar"),
    uan:         getVal("df-uan"),
    esi:         getVal("df-esi"),
    bank:        getVal("df-bank"),
    ifsc:        getVal("df-ifsc"),
...
function confirmDelete() {
  ...
  emps     = emps.filter(e => e.id !== deleteTargetId);
  saveEmployees(emps);
...
function exportEmployees() {
  ...
  const rows    = emps.map(e => [
    ...
    e.dept, e.desig,
    e.doj ? formatDisplayDate(e.doj) : "",
    e.status, e.email, e.mobile
  ]);
...
function showToast(msg, type) {
  ...
  toast.style.cssText = `
    ...
    background:${type === "success" ? "#10B981" : type === "info" ? "#6B7280" : "#EF4444"};
...
// Expose logout globally (defined in app.js)
